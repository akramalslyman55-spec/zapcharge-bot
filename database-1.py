...
class Deposit(db.Model):
    ...
    # قيم الحقول المخصصة يلي الزبون عبّاها (لو الطريقة فيها حقول مخصصة معرّفة)
    # مخزّنة كنص JSON، مثال: {"اسم صاحب الحساب": "أحمد", "رقم الهاتف": "0912345678"}
    custom_field_values = db.Column(db.Text, nullable=True)
...
class DepositMethod(db.Model):
    ...
    # لو True، الزبون لازم يرفع صورة إثبات (سكرين شوت التحويل) وقت تقديم الإيداع بهاي الطريقة
    requires_proof_image = db.Column(db.Boolean, default=False, nullable=False)

    # حقول نصية إضافية يحددها الأدمن (اختياري)، لازم الزبون يعبيها إذا كانت موجودة
    # مخزّنة كنص JSON، مثال: ["اسم صاحب الحساب", "رقم الهاتف المحول منه"]
