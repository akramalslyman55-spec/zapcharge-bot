...
with app.app_context():
    ...
    try:
        ...
        db.session.execute(text("ALTER TABLE deposit_method ADD COLUMN IF NOT EXISTS requires_proof_image BOOLEAN DEFAULT FALSE"))
        db.session.execute(text("ALTER TABLE deposit_method ADD COLUMN IF NOT EXISTS custom_fields TEXT"))
        db.session.execute(text("ALTER TABLE deposit ADD COLUMN IF NOT EXISTS custom_field_values TEXT"))
        db.session.commit()
    except Exception:
        db.session.rollback()
...
def notify_admins_new_order(order, service):
    # بتبعت إشعار فوري لكل أدمن عنده صلاحية تنفيذ الطلبات (بمن فيهم المالك/المالكين)
    recipient_ids = set(ADMIN_IDS)
    for a in Admin.query.filter_by(can_fulfill_orders=True).all():
        recipient_ids.add(a.telegram_id)

    service_name = service.name if service else "خدمة محذوفة"
    if service and service.package_name:
        service_name += f" — {service.package_name}"

    text = (
        "🛒 طلب شحن جديد بانتظار التنفيذ\n\n"
        f"الزبون: {order.user_telegram_id}\n"
        f"الخدمة: {service_name}\n"
    )
    if order.player_id:
        text += f"المعرّف: {order.player_id}\n"
    if order.quantity:
        unit = service.unit_name if service and service.unit_name else ""
        text += f"الكمية: {order.quantity} {unit}\n"
    text += f"القيمة: {order.price:.2f}$"
...
def store_order():
    ...
    notify_admins_new_order(order, service)
...
def store_deposit():
    ...
    custom_field_values = body.get("custom_field_values") or {}
    ...
    if dep_method and dep_method.requires_proof_image and not proof_image_url:
        return jsonify({"ok": False, "error": "proof_image_required"}), 400

    method_custom_fields = json.loads(dep_method.custom_fields) if dep_method and dep_method.custom_fields else []
    cleaned_custom_values = {}
    for label in method_custom_fields:
        value = str(custom_field_values.get(label, "")).strip()
        if not value:
            return jsonify({"ok": False, "error": "missing_custom_fields"}), 400
        cleaned_custom_values[label] = value
    ...
    deposit = Deposit(
        ...
        custom_field_values=json.dumps(cleaned_custom_values, ensure_ascii=False) if cleaned_custom_values else None,
        status="pending"
    )
    ...
    return jsonify([
        {
            ...
            "custom_field_values": json.loads(d.custom_field_values) if d.custom_field_values else {},
        }
        for d in deposits
    ])
        ...
        "methods": [
            {
                ...
                "currency": m.currency,
                "requires_proof_image": m.requires_proof_image,
                "custom_fields": json.loads(m.custom_fields) if m.custom_fields else [],
            }
            for m in methods
        ],
    })
...
def _parse_custom_fields(body) -> list:
    raw = body.get("custom_fields")
    if not isinstance(raw, list):
        return []
    fields = []
    for item in raw:
        label = str(item or "").strip()[:64]
        if label:
            fields.append(label)
    return fields[:10]
...
def add_deposit_method():
    ...
    custom_fields = _parse_custom_fields(body)
    ...
    method = DepositMethod(
        ...
        currency=currency,
        requires_proof_image=bool(body.get("requires_proof_image", False)),
        custom_fields=json.dumps(custom_fields, ensure_ascii=False) if custom_fields else None,
    )
...
def edit_deposit_method(method_id):
    ...
    if "requires_proof_image" in body:
        method.requires_proof_image = bool(body["requires_proof_image"])
    if "custom_fields" in body:
        custom_fields = _parse_custom_fields(body)
        method.custom_fields = json.dumps(custom_fields, ensure_ascii=False) if custom_fields else None
...
@app.route("/api/admin/settings/delete-exchange-rate", methods=["POST"])
@require_admin
def delete_exchange_rate():
    body = request.get_json(silent=True) or {}
    code = str(body.get("currency", "")).strip().upper()[:8]
    ...
    del rates[code]
    set_setting("exchange_rates", json.dumps(rates))
    log_action(request.telegram_id, "حذف سعر صرف", code)
